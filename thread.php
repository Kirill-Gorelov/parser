<?php 
/**
 * парсим только категории
 * собираем ссылки на товары с категорий
 * что бы потом парсить только товары
 * разделяй и властвуй
 */
set_time_limit(0);
ignore_user_abort(true);

include __DIR__.'/class/CurlRequest.php';
include __DIR__.'/class/Csv.php';
include __DIR__.'/class/Parser.php';
include __DIR__.'/class/FileJson.php';
include __DIR__.'/func/function.php';

const D_URL_PAGIN = 'dUrlPagin.json';
const D_URL_PAGIN_SUCCESS = 'dUrlPaginSeccess.json';
const D_URL_PAGIN_FALSE = 'dUrlPaginFalse.json';


$paginationCodeArr = FileJson::get('pagiation.json');
// var_dump($paginationCodeArr);
...
$urlPush = 'https://pushall.ru/api.php?type=self&id=113165&key=bc5ca8c3ff0d7fc5c0fd2ac03cef21f9&text=';

$parser = new Parser;

// $argOffsetStart = isset($argv[1]) && !empty($argv[1]) ? $argv[1] : 1;
// $argOffsetStop = isset($argv[2]) && !empty($argv[2]) ? $argv[2] : 1;

$re = '/<a data-product-id="(.*?)" data-testid="data-table-0-product-number" href="(.*?)">(.*?)<\/a>/m';
$reGetCount = '/<span data-testid="product-count">(.*?)<\/span>/m';
$reNoResult = '/<span class="(.*?)" data-testid="zero-results-header">No Results<\/span>/m';

$defaultPaginate = 1;
$urlCategoryFromProduct = [];
$countElementOnPage = 100;

 // filter-pluggable-cables-460.txt
  // 213.87.89.50

// $tmpCountParseUrlOneThread = 2;
for ($ii=0; $ii <= 20; $ii++) { 
    # code...
    // $argKeyUrlParse = $argv[1];
    $argKeyUrlParse = $ii;

    // $argKeyUrlParse = 1;

    $digikeyUrlCatgory = FileJson::get(D_URL_PAGIN);
    $urlParse = $digikeyUrlCatgory[$argKeyUrlParse];
    unset($digikeyUrlCatgory[$argKeyUrlParse]);
    FileJson::putArray(D_URL_PAGIN, array_values($digikeyUrlCatgory));
    $digikeyUrlCatgory = null;

    // $urlParse = 'https://www.digikey.com/en/products/filter/alarms-buzzers-and-sirens/157?s=N4IgrCBcoA5QjAGhDOl4AYMF9tA=';
    $NameFileLog = str_replace('https://www.digikey.com/en/products/', '', $urlParse);
    $NameFileLog = str_replace('/', '-', $NameFileLog);
    $NameFileLogTxt = __DIR__.'/log/pagination/'.$NameFileLog.'.txt';
    $NameFileLogCsv = __DIR__.'/log/pagination/'.$NameFileLog.'.csv';
    $NameFileLogRejectedTxt = __DIR__.'/log/pagination/NameFileLogRejectedTxt.txt';

    if (!file_exists($NameFileLogTxt)) {
        file_put_contents($NameFileLogTxt, '');
    }

    for ($i=1; $i <= $defaultPaginate; $i++) { 
        
        $paginItemCode = $i-1;
        $paginationCode = explode(':', $paginationCodeArr[$paginItemCode])['1'];
        $url = $urlParse.'?s='.$paginationCode;
        
        $responseHtml = CurlRequest::send($url, [], $headers);
        // var_dump($responseHtml);

        preg_match_all($reNoResult, $responseHtml, $matchesNoResult, PREG_SET_ORDER, 0);
        preg_match_all($re, $responseHtml, $matches, PREG_SET_ORDER, 0);

        if(!empty($matchesNoResult)){
            file_put_contents($NameFileLogTxt, "---------------------- ".PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, "Парсим адрес: ".$url.PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, "Время : ".date('d-m-Y H:i:s').PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, "Номер страницы : ".$i."/".$defaultPaginate.PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, "Результаты не найдены".PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
            file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
            //рассчитываю, что тут закончилист страницы для парсинга, если псмотри в логи
            //потому что пагинация может мудрить
            FileJson::appendItem(D_URL_PAGIN_SUCCESS, $url); 

            break;
        }

        if ($defaultPaginate == 1) {
            preg_match_all($reGetCount, $responseHtml, $matchesCountElement, PREG_SET_ORDER, 0);
            $defaultPaginate = ceil(intval(str_replace(',', '', $matchesCountElement['0']['1'])) / $countElementOnPage);
            // $defaultPaginate = $defaultPaginate > 10 ? 5 : $defaultPaginate;//TODO:тест
        }

        // var_dump($matches);
        if (empty($matches)) {
            $searchPos = strripos($responseHtml, 'The requested URL was rejected. Please consult with your administrator.');
            $searchPos403 = strripos($responseHtml, 'Click Here to Report this Error:');
            if (intval($searchPos) > 1 || intval($searchPos) > 1) {
                file_put_contents($NameFileLogRejectedTxt, $url.PHP_EOL, FILE_APPEND);
                // CurlRequest::send($urlPush.'Запрос заблокировали '.$url);
                FileJson::appendItem(D_URL_PAGIN_FALSE, $url); 

                file_put_contents($NameFileLogTxt, "---------------------- ".PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, "Парсим адрес: ".$url.PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, "Время : ".date('d-m-Y H:i:s').PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, "Номер страницы : ".$i."/".$defaultPaginate.PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, "rejected".PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
                file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
        ...
        foreach ($matches as $key => $value) {
            // $urlCategoryFromProduct[] = $value['2'];
            $tmpArrUrl[] = $value['2'];
        }
        ...
        file_put_contents($NameFileLogTxt, "Время : ".date('d-m-Y H:i:s').PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, "Номер страницы : ".$i."/".$defaultPaginate.PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, "Спарсенные адреса: ".PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, !empty($tmpArrUrl) ? arrayContext($tmpArrUrl) : 'rejected'.PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
        file_put_contents($NameFileLogTxt, ''.PHP_EOL, FILE_APPEND);
        ...
        // var_dump($NameFileLogCsv);
        $csv = new CSV($NameFileLogCsv);
        $csv->setCSV($tmpArrUrl);
        sleep(rand(10,30));
    }
    // var_dump($digikeyUrlCatgory);
    // var_dump($urlCategoryFromProduct);
    FileJson::appendItem(D_URL_PAGIN_SUCCESS, $url); 
    $defaultPaginate = 1;

    CurlRequest::send($urlPush.'Закончил парсить '.$url.' поток: '.$argKeyUrlParse.'&title='.date('d-m-Y H:i:s').' Закончил парсить  - '.$argKeyUrlParse);
